  ...
  try {
    ...
    if (!city) {
      return res.status(400).json({
        success: false,
        error:
          "City parameter is required or provide address for city extraction",
      });
    }
...
router.get("/radius-search", async (req, res) => {
  try {
    ...
    if (!coordinates) {
      ...
      console.log(
        `Found ${crimeSummaries.length} crimes for ${city} (city-based search)`
      );
    ...
    const simulatedCrimes = crimeArticles
      .map((article, index) => {
        ...
        const lngOffset =
          (distance / (111 * Math.cos((coordinates.lat * Math.PI) / 180))) *
    ...
    if (radius <= 1) {
      finalCrimes = simulatedCrimes.slice(
        0,
        Math.min(3, simulatedCrimes.length)
      );
    } else if (radius <= 2) {
      finalCrimes = simulatedCrimes.slice(
        0,
        Math.min(5, simulatedCrimes.length)
      );
    } else if (radius <= 3) {
      finalCrimes = simulatedCrimes.slice(
        0,
        Math.min(7, simulatedCrimes.length)
      );
    } else {
      finalCrimes = simulatedCrimes.slice(
        0,
        Math.min(10, simulatedCrimes.length)
      );
    }

    console.log(
      `Found ${finalCrimes.length} crimes within ${radius}km of ${address} (filtered from ${simulatedCrimes.length} total)`
    );
