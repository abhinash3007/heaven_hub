    ...
    return (
        <div className='bg-white shadow-lg hover:shadow-xl transition-shadow overflow-hidden rounded-lg w-full sm:w-[330px]'>
            <Link to={`/listing/${listing._id}`}>
                ...
                />
                {crimeCount !== null && (
                    <div className={`absolute top-2 right-2 px-2 py-1 rounded-full text-white text-xs font-semibold flex items-center gap-1 ${getCrimeSeverityColor(crimeCount)}`}>
                    ...
                    {crimeLoading ? (
                        ...
                        <div className="text-xs text-slate-600 flex items-center gap-1">
                            ...
                            {crimeCount === 0 ? (
                                ...
                                `${crimeCount} recent crime incident${crimeCount > 1 ? 's' : ''}`
                            )}
                        </div>
                    )}
                </div>
